...
export async function sendEmail(message: EmailMessage) {
  ...
  const { error } = await new Resend(apiKey).emails.send({ from, to: message.to, cc: message.cc, subject: message.subject, html: message.html, text: message.text, replyTo: message.replyTo });
