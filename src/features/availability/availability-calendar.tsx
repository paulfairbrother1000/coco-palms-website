...
import { addMonths, eachDayOfInterval, endOfMonth, format, getDay, isBefore, isSameDay, startOfMonth } from "date-fns";
...
export function AvailabilityCalendar({ ranges }: { ranges: UnavailableRange[] }) {
  ...
  function choose(date: Date) {
    if (isBeforeEarliestArrival(date, now) || unavailable(date) || isTooShortDeparture(date, arrival)) return;
  ...
  return <div className="calendar-shell">
    ...
    <div className="calendar-grid">{Array.from({ length: getDay(startOfMonth(month)) }, (_, index) => <span key={`empty-${index}`} />)}{days.map((date) => {
      const blocked = unavailable(date) || isBeforeEarliestArrival(date, now) || isTooShortDeparture(date, arrival); const selected = (arrival && isSameDay(date, arrival)) || (departure && isSameDay(date, departure));
      return <button key={date.toISOString()} disabled={blocked} aria-label={`${format(date,"MMMM d, yyyy")}${blocked ? ", unavailable" : ", available"}`} className={selected ? "selected" : blocked ? "blocked" : ""} onClick={() => choose(date)}>{format(date,"d")}</button>;
    })}</div>
