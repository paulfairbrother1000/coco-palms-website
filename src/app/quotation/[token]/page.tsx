...
export default async function QuotationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const confirmation = {
    ...
    adults: quote.adults,
    childrenSixToSeventeen: quote.childrenSixToSeventeen,
    childrenUnderSix: quote.childrenUnderSix,
    nights: quote.calculation.nights,
    quotationTotal: quote.calculation.quotationTotal,
    dueToConfirm: quote.calculation.dueToConfirm,
    securityDeposit: quote.calculation.securityDeposit,
  };
  ...
  return <>
    ...
    <section className="section quotation-layout">
      ...
      <QuoteResult calculation={quote.calculation} publicToken={quote.token} confirmation={confirmation} disabled={expired} />
    </section>
  </>;
}
