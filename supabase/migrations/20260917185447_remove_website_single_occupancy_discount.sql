...
begin
  ...
  with priced as (
    ...
    join lateral (
      ...
      where rp.property_id = p_property_id
        and rp.is_active
        and (
          (rp.is_annual_recurring and public.month_day_in_period(d::date, rp.start_date, rp.end_date))
    ...
    select name, per_night, count(*) nights, sum(per_night) total, min(stay_date) first_date
    from priced
    group by name, per_night
  )
  select sum(total), jsonb_agg(
    jsonb_build_object('period', name, 'nights', nights, 'rate', per_night, 'total', total)
    order by first_date
  )
  into v_base, v_rate_breakdown
  from grouped;
  ...
  if lower(coalesce(p_optional_discount, '')) = 'two-week' then
    v_secondary_type := 'two-week';
    v_secondary_rate := v_policy.notice_2_weeks_discount;
  elsif lower(coalesce(p_optional_discount, '')) = 'four-week' then
    v_secondary_type := 'four-week';
    v_secondary_rate := v_policy.notice_4_weeks_discount;
  elsif lower(coalesce(p_optional_discount, '')) = 'early-bird' then
    v_secondary_type := 'early-bird';
    v_secondary_rate := v_policy.early_bird_discount;
  end if;
  ...
  return jsonb_build_object(
    ...
    'under6_count', p_under6_count,
    'levy_guests', v_levy_guests,
    'base_total', v_base,
    'blended_rate', v_blended,
    'rate_breakdown', coalesce(v_rate_breakdown, '[]'::jsonb),
    'long_stay_discount', v_long_discount,
    'secondary_discount_type', v_secondary_type,
    'secondary_discount', v_secondary_discount,
    'discount_total', v_long_discount + v_secondary_discount,
    'discounted_base', v_discounted_base,
    'short_stay_levy', v_short_stay_levy,
    'abst_total', v_abst,
    'levy_total', v_levy,
    'fees_total', v_fees,
    'quotation_total', v_total,
    'security_deposit', v_security,
    'deposit_due', round(v_total * v_policy.deposit_percentage, 2),
    'balance_due', round(v_total * (1 - v_policy.deposit_percentage), 2),
